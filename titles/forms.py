...
class TitleForm(ModelForm):
    class Meta:
        model = Title
        fields = ["title", "title_kana", "short_title", "content", "genre",
                  "sub_genre", "season", "air_date", "website", "related_titles", "tags"]
...
class EpisodeForm(ModelForm):
    class Meta:
        model = Episode
        fields = ["episode_title", "episode_number",
                  "content", "air_date", "duration", "tags",]
        widgets = {"air_date": forms.DateTimeInput(
...
class SourceSearchForm(forms.Form):
    titles = forms.MultipleChoiceField(
        label="追加するタイトルを選択", widget=forms.CheckboxSelectMultiple, choices=[])
