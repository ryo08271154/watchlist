...
class Title(models.Model):
    ...
    title_kana = models.CharField(
        max_length=200, blank=True, verbose_name="タイトルふりがな")
    short_title = models.CharField(
        max_length=50, blank=True, verbose_name="タイトル略称")
    content = models.TextField(blank=True, verbose_name="概要")
    genre = models.ForeignKey(
        Genre, on_delete=models.PROTECT, verbose_name="ジャンル")
    sub_genre = models.ManyToManyField(
        SubGenre, blank=True, verbose_name="サブジャンル")
    ...
    related_titles = models.ManyToManyField(
        "self", blank=True, verbose_name="関連タイトル")
...
class Episode(models.Model):
    ...
    title = models.ForeignKey(
        Title, on_delete=models.CASCADE, related_name="episodes", verbose_name="タイトル")
    episode_title = models.CharField(
        max_length=200, blank=True, verbose_name="エピソードタイトル")
    ...
    def __str__(self):
        return f"{self.title.title} - {self.episode_number}"
