...
class BaseExportView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        self.fields = request.GET.getlist(
            "fields") if request.GET.getlist("fields") else []
        filter_kwargs = self.get_filter_kwargs()
        export_data = self.model.objects.filter(
            **filter_kwargs).order_by(self.order_by).distinct()
...
def tags_add(title, field):
    ...
    if tags_name:
        for tag in tags_name:
            ...
            tags.append(tag)
        title.tags.add(*tags)
        setattr(title, field, re.sub(r"#.*\s*", "",
                getattr(title, field)))  # コメントからタグの部分を消す
        title.save()
    return tags
        ...
        for r in reader:
            ...
            try:
                ...
                if r[sub_genre_column] != "":
                    sub_genre, created = SubGenre.objects.get_or_create(
                        name=r[sub_genre_column])
                else:
                    sub_genre = None
    ...
    def form_valid(self, form):
        ...
        for title in titles:
            related_titles = Title.objects.filter(
                title__icontains=title.title).exclude(id=title.id)
            if related_titles.exists():
                title.related_titles.add(*related_titles)
            for related_title in title.related_titles.all():
                title.related_titles.add(*related_title.related_titles.all().exclude(
...
class TitleEpisodeSourceImportView(LoginRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = get_object_or_404(Title, id=self.kwargs["pk"])
        context["source_select_form"] = SourceSelectForm(
            initial={"q": get_object_or_404(Title, id=self.kwargs["pk"]).title})
        return context
