...
class WatchRecord(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="ユーザー", editable=False)
    title = models.ForeignKey(
        Title, on_delete=models.PROTECT, verbose_name="タイトル")
    comment_title = models.CharField(
        max_length=200, verbose_name="コメントタイトル", blank=True)
    comment = models.TextField(blank=True, verbose_name="コメント")
    watched_date = models.DateField(blank=True, null=True, verbose_name="視聴日")
    rating = models.PositiveIntegerField(blank=True, null=True, validators=[
                                         MinValueValidator(1), MaxValueValidator(100)], verbose_name="評価")
    is_spoiler = models.BooleanField(default=False, verbose_name="ネタバレ")
    status = models.CharField(
        choices=WATCH_STATUS_CHOICES, max_length=20, blank=True, verbose_name="視聴状況")
    watch_method = models.ForeignKey(
        WatchMethod, on_delete=models.PROTECT, blank=True, null=True, verbose_name="視聴方法")
...
class EpisodeWatchRecord(models.Model):
    ...
    episode = models.ForeignKey(
        Episode, on_delete=models.PROTECT, verbose_name="エピソード")
    comment_title = models.CharField(
        max_length=200, verbose_name="コメントタイトル", blank=True)
    comment = models.TextField(blank=True, verbose_name="コメント")
    watched_date = models.DateField(blank=True, null=True, verbose_name="視聴日")
