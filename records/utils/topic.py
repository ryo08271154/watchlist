...
def recommended_topic(request):
    ...
    watched_list = WatchRecord.objects.filter(user=request.user,status="watched").order_by("-updated_at","-watched_date")[:10] #直近10件の視聴履歴を取得
    ...
    watched_genre_list = list(watched_list.values_list("title__genre",flat=True))
    watched_sub_genre_list = list(watched_list.values_list("title__sub_genre",flat=True))
    ...
    def get_score(title):
        score = 0
        title_avg_rating = WatchRecord.objects.filter(title=title).aggregate(Avg("rating"))["rating__avg"]
        #平均評価に近い場合は加点
        if title_avg_rating:
        ...
        if last_watched_date <= datetime.date.today() - datetime.timedelta(days=365):
            score += 3 * min(datetime.date.today().year - last_watched_date.year,5) #5年以上は5点
        elif last_watched_date >= datetime.date.today() - datetime.timedelta(days=90):
            score -= 5
        #関連タイトルに最近見ている場合は減点
        for related_title in title.related_titles.all():
            related_last_watched = WatchRecord.objects.filter(user=request.user,title=related_title,status="watched").order_by("-watched_date").first()
            if related_last_watched:
                ...
                if related_last_watched_date >= datetime.date.today() - datetime.timedelta(days=90):
                    score -= max((datetime.date.today() - related_last_watched_date).days,0) // 90
